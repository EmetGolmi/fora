class OfficialsController < ApplicationController
  ...
  HARDCODED_VOTING_STATS = {
    "F000479" => { missed_votes_pct: 13.9, party_vote_pct: 68.0 },
    "M001243" => { missed_votes_pct:  2.8, party_vote_pct: 95.0 },
    "E000296" => { missed_votes_pct:  8.1, party_vote_pct: 96.0 },
    "B001296" => { missed_votes_pct:  5.2, party_vote_pct: 97.0 }
  }.freeze
  ...
  def show
    ...
    member_response = self.class.get("/member/#{bioguide_id}",
    ...
    threads << Thread.new do
      ...
      begin
        resp = HTTParty.get(
          ...
          query:   { action: "query", titles: "File:#{wiki_filename}",
                     prop: "imageinfo", iiprop: "url", iiurlwidth: 960,
                     format: "json" },
          headers: { "User-Agent" => "FORA/1.0 (fora.center)" }
        )
        photo = resp.success? ? resp.parsed_response
                                    .dig("query","pages")
                                    &.values&.first
                                    &.dig("imageinfo", 0, "thumburl") : nil
    ...
    threads << Thread.new do
      ActiveRecord::Base.connection_pool.with_connection do
        ...
        bill_id_map = CivicBill.where(identifier: identifiers)
                               .pluck(:identifier, :id).to_h
          ...
          rows = unmatched.map do |bill|
            raw_status = bill.dig("latestAction", "text").to_s.presence
            {
              ...
              status_date:  bill.dig("latestAction", "actionDate")
                                .then { |d| Date.parse(d) rescue nil },
              jurisdiction: "federal",
              created_at:   now,
              updated_at:   now
            }
          end
          CivicBill.upsert_all(rows, unique_by: %i[source external_id],
                               update_only: %i[title status bill_stage status_date])
          bill_id_map = CivicBill.where(identifier: identifiers)
                                 .pluck(:identifier, :id).to_h
        end
    ...
    # Committees
    threads << Thread.new do
      resp = self.class.get("/member/#{bioguide_id}/committees",
                            query: { api_key: api_key })
      committees = if resp.success?
        raw     = resp.parsed_response["committeeHistory"] ||
                  resp.parsed_response["committees"] || []
        current = raw.select { |c| c["endDate"].nil? }
        (current.any? ? current : raw).first(4).map do |c|
          { name: c.dig("committee","name") || c["name"] ||
    ...
    # Voting stats — hardcoded, no GovTrack scrape
    threads << Thread.new do
    ...
    threads << Thread.new do
      begin
        news_resp = HTTParty.get(
          ...
          query: { apiKey: ENV["NEWSAPI_KEY"],
                   q: "\"#{member_name.split.last}\"",
                   language: "en", sortBy: "publishedAt", pageSize: 5 }
        )
        mutex.synchronize do
          @news_articles = news_resp.success? ?
            (news_resp.parsed_response["articles"] || []) : []
        end
      rescue
    ...
    @finance = OfficialFinanceSummary
                 .where(fec_candidate_id: fec_cand_id)
                 .order(cycle_year: :desc)
                 .first if fec_cand_id

    @social_handles = {
      "F000479" => { x: "SenJohnFetterman", instagram: "senjohnfetterman",
                     youtube: nil, website: "https://www.fetterman.senate.gov" },
      "M001243" => { x: "SenMcCormickPA",   instagram: "senmccormickpa",
                     youtube: "SenMcCormickPA", website: "https://www.mccormick.senate.gov" },
      "E000296" => { x: "RepDwightEvans",    instagram: "repdwightevans",
                     youtube: nil, website: "https://evans.house.gov" },
      "B001296" => { x: "RepBrendanBoyle",   instagram: nil,
                     youtube: nil, website: "https://boyle.house.gov" }
    }[bioguide_id] || {}
  end
  ...
  def state_show
    ...
    # Try hardcoded fallback first — avoids OpenStates rate limits entirely
    person = STATE_OFFICIALS[openstates_id]

    # If not hardcoded, try the API
    if person.nil?
      begin
        person_response = HTTParty.get(
          "https://v3.openstates.org/people/#{openstates_id}",
          query:   { apikey: ENV["OPENSTATES_API_KEY"] },
          timeout: 8
        )
    ...
    begin
      bills_response = HTTParty.get(
        ...
        query:   { sponsor_id: openstates_id,
                   apikey: ENV["OPENSTATES_API_KEY"],
                   sort: "updated_desc", per_page: 5 },
        timeout: 8
      )
      @bills = bills_response.success? ?
        (bills_response.parsed_response["results"] || []) : []
    rescue
      @bills = []
    end

    identifiers  = @bills.map { |b| b["identifier"] }
    @bill_id_map = CivicBill.where(identifier: identifiers, jurisdiction: "pennsylvania")
                            .pluck(:identifier, :id).to_h
...
Save the file, then in Claude Code:
```
git add app/controllers/officials_controller.rb
git commit -m "Fix state profiles 404 + hardcode voting stats — remove GovTrack scraper"
git push
